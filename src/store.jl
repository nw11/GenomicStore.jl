...
#=
if file_type == "bedgraph_cpg" && storage_type == "score"
            (path,ext) = splitext(file_path)
            gzip = (ext==".gz" || ext==".gzip" ) ? true : false
            save_bedgraph_track(genomic_store_path,file_path,track_id,chr_sizes_path;
                                start_coord_shift=coord_shift, OUT_OF_RANGE_VAL=na_val, gzip=gzip)
            Lumberjack.info("Saved track $track_id")
        elseif file_type == "point_cpg" && storage_type == "point"
            save_point_start_track(genomic_store_path,file_path,track_id,chr_sizes_path;start_coord_shift=coord_shift,strand_filter_char="-")
        elseif file_type == "bed_methpipe" && storage_type == "interval"
            save_interval_track(genomic_store_path, file_path, track_id, chr_sizes_path; start_coord_shift=coord_shift, strand_filter_char=nothing)
        elseif file_type == "bedgraph" && storage_type == "interval"
            save_interval_track(genomic_store_path, file_path, track_id, chr_sizes_path; start_coord_shift=coord_shift,strand_filter_char=nothing)
        elseif file_type == "bedgraph" && storage_type == "interval_multiclass_int8"
            save_interval_track(genomic_store_path, file_path, track_id, chr_sizes_path; start_coord_shift=coord_shift,strand_filter_char=nothing,interval_label_type="multiclass_int8")
        elseif file_type == "methpipe_cpg_bed_levels" && storage_type == "score"
            (path,ext) = splitext(file_path)
            gzip = (ext==".gz" || ext==".gzip" ) ? true : false
            save_bed_track(genomic_store_path,file_path,track_id,chr_sizes_path;
                                start_coord_shift=coord_shift, OUT_OF_RANGE_VAL=na_val, gzip=gzip,
                                bedtype=file_type
                           )
            Lumberjack.info("Saved track $track_id")
        elseif file_type == "methpipe_cpg_bed_coverage" && storage_type == "count"
           (path,ext) = splitext(file_path)
            gzip = (ext==".gz" || ext==".gzip" ) ? true : false
            save_bed_track(genomic_store_path,file_path,track_id,chr_sizes_path;
                                start_coord_shift=coord_shift, OUT_OF_RANGE_VAL=na_val, gzip=gzip,
